...
class LYT_RGB(nn.Module):
    def __init__(self, filters=32):
        ...
        self.denoiser_r = Denoiser(filters // 2)
        self.denoiser_g = Denoiser(filters // 2)
        ...
        self.ref_conv = nn.Conv2d(filters * 3, filters, kernel_size=1, padding=0)
        self.msef = MSEFBlock(filters)
    ...
    def forward(self, inputs):
        ...
        r = self.denoiser_r(r) + r
        g = self.denoiser_g(g) + g
        ...
        ref = torch.cat([r_processed, g_processed, b_processed], dim=1)
        ref = self.ref_conv(ref)
        shortcut = ref
